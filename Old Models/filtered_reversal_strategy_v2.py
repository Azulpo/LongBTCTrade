...
"""
Script: filtered_reversal_strategy_v2.py

Description:
Simulates a filtered BTC reversal trading strategy using 1-minute price data. The strategy enters long trades when 15-minute momentum is negative but only proceeds if at least 4 of 5 market condition filters are met:
1. Low volatility
2. Short-term upward bounce
3. No new 1-hour low
4. No extreme selling in the past hour
5. Uptrend confirmation using 4-hour SMA

Exit conditions include:
- 3.47% trailing stop from peak price
- 3% drop within 5 minutes

The simulation calculates performance metrics including final balance, Sharpe ratio, max drawdown, and compares the strategy to a BTC buy-and-hold benchmark. It also tracks and reports fee impact.

This script helps evaluate the effectiveness of multi-condition filtered reversal strategies for spot BTC trading.
"""
